...
interface DisplayNameModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (displayName: string, isFaculty: boolean, passphrase?: string) => void;
  title: string;
  actionText: string;
...
export const DisplayNameModal: React.FC<DisplayNameModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setError('');
    onSubmit(cleanName, isFaculty, isFaculty ? passphrase.trim() : undefined);
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={title}>
      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          <label className="block text-footnote font-medium text-apple-textSecondary dark:text-white/70 mb-2">
            Your Display Name
          </label>
          ...
          <p className="text-caption text-apple-textSecondary dark:text-white/50 mt-1.5">
            Visible only in this session. Never saved.
          </p>
        </div>

        <div className="p-3.5 bg-apple-secondaryBg/80 dark:bg-white/5 rounded-ios-btn border border-apple-border/40 dark:border-white/10">
          ...
          {isFaculty && (
            <div className="mt-3 pt-3 border-t border-apple-border/50 dark:border-white/10 animate-fade-in">
              <label className="block text-footnote text-apple-textSecondary dark:text-white/70 mb-1.5">
                Faculty Passphrase
              </label>
              <input
                type="password"
                ...
                className="w-full px-3.5 py-2.5 bg-white dark:bg-[#1C1C1E] rounded-ios-input text-body text-apple-textPrimary dark:text-white placeholder:text-apple-textSecondary/50 dark:placeholder:text-white/30 outline-none border border-apple-border dark:border-white/10 focus:ring-2 focus:ring-apple-blue/80 transition-all text-sm"
              />
            </div>
          )}
        </div>
